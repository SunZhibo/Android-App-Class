...
public class ClockView extends View {
    ...
    private static final float OUTSIDE_CIRCLE_LENGTH_RATIO = 0.03f; // 中心圆边缘相对表盘半径的比例
    private static final float INSIDE_CIRCLE_LENGTH_RATIO = 0.025f; // 中心圆相对表盘半径的比例
    private static final float NUMBER_CIRCLE_LENGTH_RATIO = 0.8f; // 数字相对表盘半径的比例
    private static final float DOT_LENGTH_RATIO = 0.91f; // 点相对表盘半径的比例
    ...
    private static final float OUTSIDE_CIRCLE_WIDTH = 6; // 中心圆的边缘宽度
    private static final float NUMBER_TEXT_SIZE = 80; // 数字的字号
    private static final float DOT_RADIUS = 7; // 点大小
    ...
    private void drawCircle(Canvas canvas) {
        circlePaint.setStrokeWidth(OUTSIDE_CIRCLE_WIDTH);
        circlePaint.setColor(Color.WHITE);
        circlePaint.setStyle(Paint.Style.STROKE);
        canvas.drawCircle(centerX, centerY, radius * OUTSIDE_CIRCLE_LENGTH_RATIO, circlePaint);

        circlePaint.setColor(Color.rgb(203, 204, 203));
        circlePaint.setStyle(Paint.Style.FILL);
        canvas.drawCircle(centerX, centerY, radius * INSIDE_CIRCLE_LENGTH_RATIO, circlePaint);
    }
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        double secondDegree = (float)second * 6 - 90;
        float secondEndX = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(secondDegree)));
        float secondEndY = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(secondDegree)));
        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        needlePaint.setColor(Color.rgb(203, 204, 203));
        canvas.drawLine(centerX, centerY, secondEndX, secondEndY, needlePaint);

        double minuteDegree = ((float)minute + (float)second / 60) * 6 - 90;
        float minuteEndX = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(minuteDegree)));
        float minuteEndY = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(minuteDegree)));
        ...
        float hourDegree = ((float)hour + (float)minute / 60 + (float)second / 3600) * 30 - 90;
        float hourEndX = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(hourDegree)));
        float hourEndY = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(hourDegree)));
    ...
    private void drawTimeNumbers(Canvas canvas) {
        ...
        for (int i = 1; i <= 12; i++) {
            ...
            float numberDegree = i * 30 - 90;
            float numberX = (float) (centerX + radius * NUMBER_CIRCLE_LENGTH_RATIO * Math.cos(Math.toRadians(numberDegree)));
            float numberY = (float) (centerX + radius * NUMBER_CIRCLE_LENGTH_RATIO * Math.sin(Math.toRadians(numberDegree)));
            Rect rect = new Rect();
    ...
    private void drawDot(Canvas canvas) {
        // 用点绘制表盘刻度
        for (int i = 1; i <= 12; i++) {
            float dotDegree = i * 30 - 90;
            float dotX = (float) (centerX + radius * DOT_LENGTH_RATIO * Math.cos(Math.toRadians(dotDegree)));
            float dotY = (float) (centerX + radius * DOT_LENGTH_RATIO * Math.sin(Math.toRadians(dotDegree)));
